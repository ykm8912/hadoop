...
public class MovieAverageRateTopK extends Configured implements Tool {
    ...
    public static class TopKMapper extends Mapper<Object, Text, Text, Text> {
        ...
        protected void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            String[] columns = value.toString().split("\t");
            Double k = Double.parseDouble(columns[1]);
            
            // 같은 평점이 있을 경우, 영화 제목을 $$로 구분하여 붙여준다.
            // ex) 4.0, Toy Story (1995)$$Jumanji (1995)
            if (topKMap.containsKey(k)) {
                topKMap.put(Double.parseDouble(columns[1]), new Text(topKMap.get(k).toString() + "$$" + columns[0]));
            }else{
                // 같은 평점이 없을 경우, 그냥 영화 제목을 넣어준다.
